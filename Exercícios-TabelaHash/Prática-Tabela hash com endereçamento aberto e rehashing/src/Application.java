	...
	public static void main(String[] args) {
		...
		while(!linha.equals("FIM")) {
		 ...
		 while(!linha.equals("FIM")) {
				...
				System.out.println(pesq.getPosicao() + " SIM");
				}
				linha = entrada.next();
			}
...
class Accommodation {
	...
	private int reviews, accommodates, roomId, hostId, posicao;
...
class TabelaHash {
	...
	private int M;
	private Accommodation[] tabelaHash;
	
	public TabelaHash(int tamanho) {
		
		M = tamanho;
		
		tabelaHash = new Accommodation[M];
	...
	// h(x) = ((x + k) % M)
	private int funcaoHash(int chave, int k) {
		
		return (chave % M + k * (chave % 3803)) % M;
	}
	...
	public Accommodation pesquisar(int chave) {
		...
		if (encontrou) {
			if(pesquisado != null) {
				pesquisado.Setposicao((chave % M + k * (chave % 3803)) % M);
				return pesquisado;
			}else
			return pesquisado;
		}
