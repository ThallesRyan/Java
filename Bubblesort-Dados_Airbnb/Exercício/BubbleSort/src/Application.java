			...
			while(line != null) {	
				...
				for(int i = 0; i < sortArray.length; i++) {
					...
					for(int j = 0; j <(sortArray.length - 1); j++) {
						...
						comparacaoTotal++;
						if(sortArray[j].getOverallSatisfaction() > sortArray[j + 1].getOverallSatisfaction()) {
							aux = sortArray[j];
							sortArray[j] = sortArray[j + 1];
							sortArray[j + 1] = aux;
							mudancasTotal ++;
							controle = false;	
						}else if(sortArray[j].getOverallSatisfaction() == sortArray[j + 1].getOverallSatisfaction()) {
							comparacaoTotal+=2;
							if(sortArray[j].getRoomId() > sortArray[j + 1].getRoomId()) {
								aux = sortArray[j];
								sortArray[j] = sortArray[j + 1];
								sortArray[j + 1] = aux;
								mudancasTotal ++;
				...
				File relatorio = new File("737156_bolha.txt");
